...
parser.add_argument('--set', type=str, default='IDRiD', help='location of the data corpus')
...
parser.add_argument('--iterations', type=int, default=3, help='T')
...
class ImageFolderWithSkip(ImageFolder):
    def __getitem__(self, index):
        try:
            ...
            # If there is a damaged file, ignore and continue to read the next file
...
def main():
  ...
  valid_queue = torch.utils.data.DataLoader(valid_dataset, batch_size=args.batch_size // 2 , shuffle=True)
  
  infer_queue = torch.utils.data.DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=True)
...
def CFF(model,inputs,target,pop_sample,criterion,ori_pre,args):
    ...
    if isinstance(arch_params, list) or isinstance(arch_params, tuple):
        arch_params = torch.stack(arch_params, dim=0)

    loss_b = torch.mean(torch.logsumexp(arch_params, dim=-1))#Beta-Decay
    if ori_pre <= prec1:
        loss = loss + similar_loss*args.mon + loss_b
    else:
        loss = loss - similar_loss*args.mon + loss_b
...
def explore_architecture(valid_queue, model, criterion, epoch, num_samples):
    ...
    with torch.no_grad():
        ...
        bounds = [(-1, 1)] * (2 * model.num_edges * model.num_ops)
        
        def objective_function(weights):
            normal_weights = weights[:model.num_edges * model.num_ops].reshape((model.num_edges, model.num_ops))
            reduce_weights = weights[model.num_edges * model.num_ops:].reshape((model.num_edges, model.num_ops))
            
            loss, _, _ = CFF(model, x, y, torch.tensor([normal_weights, reduce_weights]).cuda(), criterion, ori_pre, args)
            return loss.item()  
        
        result = differential_evolution(objective_function, bounds, maxiter=num_samples, popsize=50, seed=1)
        
        best_weights = result.x
        best_normal_weights = best_weights[:model.num_edges * model.num_ops].reshape((model.num_edges, model.num_ops))
        best_reduce_weights = best_weights[model.num_edges * model.num_ops:].reshape((model.num_edges, model.num_ops))
        
        logging.info("loss = %f", result.fun)  

        return best_normal_weights, best_reduce_weights
